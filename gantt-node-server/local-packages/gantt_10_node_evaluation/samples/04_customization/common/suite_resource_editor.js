function initResourceEditForm() {
	gantt.$lightboxControl.resources.addForm = function () {
		...
		this.deleteAllResources = function () {
			...
			resourceData = [];
			gantt.$resourceStore.parse(resourceData)
			dhx.awaitRedraw().then(function () {
				gantt.$lightboxControl.fillTabContent("resources");
			});
		};
				...
				if (el.$id == row.$id && column.id != "resource_id") {
					...
					if (+el.start_date >= el.end_date) {
						if (column.id == "start_date") {
							el.end_date = gantt.calculateEndDate({ start_date: el.start_date, duration: el.duration, unit: "hour" });
							row.end_date = el.end_date;
						}
						else {
							el.start_date = gantt.calculateEndDate({ start_date: el.end_date, duration: -el.duration, unit: "hour" });
							row.start_date = el.start_date;
						}
					}
					if (column.id == "value" && +value < 0) {
						row.value = Math.abs(value);
						el[column.id] = row.value;
					}
				}
			})
		});
		...
		gantt._resourceEditor.events.on("AfterEditEnd", function (value, row, column) {
			...
			if (column.id == "parent") {
				var scrollPosition = gantt.$lightboxControl.resources.saveScrollPosition();
