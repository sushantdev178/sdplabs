...
// ────────────────────────────────────────────────
// HIERARCHICAL IDS GENERATOR (Laravel Context)
// ────────────────────────────────────────────────
...
const recalculateScope = ({
    ...
    // Isolated deep object copy to prevent runtime contamination
    let tasksForScheduling = tasksInScope.map(t => ({ ...t }));

    // 1. Process snap to working days
    ...
    // Handle structural layout expansion
    const expandTargets = (!config.manage_linked_hierarchies && triggeredHierarchyIds)
        ? afterGanttArray.filter(t => triggeredHierarchyIds.has(t.id))
        : afterGanttArray;
...
export const recalculateImpact = async ({
    ...
    // 🔍 FIXED: Removed non-existent manage_linked_hierarchies field from SELECT query
